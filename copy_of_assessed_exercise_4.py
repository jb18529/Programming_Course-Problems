# -*- coding: utf-8 -*-
"""Copy of Assessed exercise 4.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1IICoVcQD110ekVG9TvtHTpfhn20aorFu
...
1. Implement a child class `Noun`.  Nouns should have the following eligible forms:
...
2. Not all the nouns are countable; some nouns in English do not have the plural form.  Implement class `UncountableNoun` inheriting from `Noun`.  The only eligible form of an `UncountableNoun` is the basic form.

3. There are some exceptions from the above cases, e.g. the plural of "mouse" is "mice".  Implement a class `IrregularNoun` inheriting from `Noun` which takes two parameters in the contructor: the basic form and the plural form.  These will be the only eligible forms of `IrregularNoun`.  Note that it is allowed that both forms are the same, e.g. the plural of "aircraft" is "aircraft".

4. Implement class `Verb` inheriting from `WordBase`.  Its constructor should take only one parameter – the basic form.  The eligible forms of a verb are as follows:
  * The basic form
...
5. Implement class `IrregularVerb` inheriting from `Verb`, to represent verbs such as `go`, `eat`, etc.  The constructor of `IrregularVerb` should take three parameters: the basic form, past tense and past participle.  
...
6. Implement class `ToBe` inheriting from `Verb`.  Its constructor should not take any parameters.  Its eligible forms are "be", "am", "is", "are", "was", "were", "been", and "beeing".  The `basic_form` variable of this object should be "be".
...
7. Having a hierarchy of classes representing English words, you can now create a spellchecker.  Write a class `Spellchecker` that will maintain a dictionary of known words (I do not mean the dictionary datatype!) and implement the following methods:
  ...
  * Method `is_correct_sentence` that takes a single parameter (the sentence) and returns a Boolean: `True` if every word in the sentence is known and `False` otherwise.  Assume that a word is known if it is one of the eligible forms of one of the known words.
  * Method `find_matches` that takes two parameters: a word (a string) and a list of `WordComparator` objects (see the provided code).  The method should return a list of all the known words or their eligible forms that match the given word according to at least one of the given `WordComparator` objects.  More on word comparators later.
...
8. Implement `IdenticalWordComparator` class (a subclass of `WordComparator`) that matches only the words that are to-the-letter identical.  E.g., it should match "cat" and "cat" but should not match "cat" and "cab".

9. Implement `OneSymbolDiffComparator` class (a subclass of `WordComparator`) that matches only the words that are different in exactly one letter.  E.g., it should match "cat" and "cab" but should not match any of "cat" and "cate", "cat" and "but", or "cat" and "cat".

10. Implement `SwapTwoSymbolsComparator` class (a subclass of `WordComparator`) that matches only the words where two consecutive letters are swapped.  For example, it should match "cheese" and "cheees", or "cheese and "hceese", but it should not match "cheese" and "echese", or "cheese" and "cheese" (if the word has a double letter, such as "ee" in "cheese", ignore it so that the comparator would not match the word itself).
...
class Noun(WordBase):
  #look at constructors in python and which would automatically alter basic_form to plural
  #assuming noun only take basic form as input but can take plural form as well
  ...
  def get_forms(self):
    ...
    '''
    if self.basic_form[-1] == "s":
      self.plural_form = self.basic_form + "es"
      return self.plural_form
    else:
      self.plural_form = self.basic_form + "s"
      return self.plural_form
    '''
...
class Verb(WordBase):
  ...
  def get_forms(self):
    #check all forms for verb, add to list and return list including basic forms and all other forms
    verb_forms = []
    ...
    verb_plural_s = ""
    if self.basic_form[-1] == "s":
      verb_plural_s = self.basic_form + "es"
    else:
      verb_plural_s = self.basic_form + "s"

    verb_plural_ing = ""
    if self.basic_form[-1] == "e":
      verb_plural_ing = self.basic_form[:-1] + "ing"
    else:
      verb_plural_ing = self.basic_form + "ing"

    verb_plural_ed = ""
    if self.basic_form[-1] == "e":
      verb_plural_ed = self.basic_form + "d"
    else:
      verb_plural_ed = self.basic_form + "ed"
    '''



'''
Implement class IrregularVerb inheriting from Verb, to represent verbs such as go, eat, etc. 
The constructor of IrregularVerb should take three parameters: the basic form, past tense and past participle.

The basic and the past forms should be a compulsory parameters but the past participle should be optional. 
For example, one may want to write IrregularVerb("spend", "spent"), 
which should be interpreted as a word with basic form "spend", past form "spent" and past participle form "spent".

The eligible forms of IrregularVerb are as follows:

    The basic form
    The basic form with added "s" (or "es" if the last letter in the basic form is "s")
    The basihow to store multiple instances of a class within a classc form with added "ing"; if the last letter of the basic form is "e", exclude it (e.g., "move" -> "moving")
    The past form
    The past participle form
'''
...
class Spellchecker():
  ...
  def add_word(self, class_object):
    ...
    '''
    Method delete_word that takes two parameters: the basic form of a word and a class. 
    The method should delete all the words from the dictionary of known words that have the given basic form 
    and are of the given class (or a subclass of the given class). 
    For example, if the dictionary includes an irregular verb "put", calling my_spellcheker.delete_word("put", Verb) should delete it, 
    but calling my_spellcheker.delete_word("put", Noun) should not delete it.
    Also, if the dictionary includes an irregular verb "put" and a regular verb "put" then my_spellcheker.delete_word("put", Verb) 
    should delete both of them.
    '''
  def delete_word(self, basic_form, class_input):
    ...
    for types in Spellchecker.class_types:
      ...
      i += 1
    #return Spellchecker.class_types
  
  '''
  Method is_correct_sentence that takes a single parameter (the sentence) 
  and returns a Boolean: True if every word in the sentence is known and False otherwise. 
  Assume that a word is known if it is one of the eligible forms of one of the known words.
  '''
  def is_correct_sentence(self, the_sentence):
    ...
    if len(match_result) == len(new_sentence) and False not in match_result:
      return True
    else:
      return False
    
    '''
Method find_matches that takes two parameters: a word (a string) and a 
list of objects inheriting from WordComparator (see the provided code). 
The method should return a list of strings. 
Specifically, out of all the elegible forms of all the known words, 
it should return those elegible forms that match the given word according to at least one of the given WordComparator objects. 
For example, if the known words are nouns 'cat' and 'dog', and the comparator only accepts exact matches, 
then if should return ["cats"] if the given word is "cats".

find_matches is allowed to return several identical strings. 
For example, assume that the list of known words includes verb "ride" and noun "ride". 
Then find_matches for the word "ride", with appropriate comparator, may return ["ride", "ride"], however ["ride"] will also be acceptable.
    '''
  def find_matches(self, a_word, word_Comparator):
    #word_Comparator is an object like Noun, Verb, IrregularVerb
    result_list = []
    for typ in word_Comparator:
      for i in Spellchecker.known_words:
        for word in i:
          if typ.match(a_word, word) == True: #is it typ.match(a_word, word) ??
            result_list.append(word)
    return result_list
  '''
  Method autocorrect_word that takes two parameters: a word (a string) and a list of WordComparator objects. 
  The method should perform as follows:

    Find all the eligible forms of all the known words that match the given word according to the first given WordComparator object. 
    If there are such matches, sort them alphabetically and return the first of them. 
    E.g., if the matches are "pat", "cab" and "cat", you should return "cab".

    If there are no such matches, repeat the process for the second given comparator object.
    If there are still no matches, repeat the process for the third given comparator object, etc.
    If no matches found for any of the given comparator objects, return the original word.

    The idea is that the list of comparator objects will usually begin with comparators 
    that only allow small mistakes and end with comparators that allow bigger mistakes. 
    Then the autocorrect_word method will find a match that is closest to the given word. 
    E.g., given appropriate comparators, it would autocorrect "pleese" as "please" and not as "cheese".
  '''
  ...
  '''
  Method autocorrect_sentence that takes two parameters: a sentence (a string) and a list of WordComparator objects. 
  The method should 'autocorrect' each word in the sentence according to the logic of the autocorrect_word method. 
  It should return a new sentence where each word is the autocorrected version of the corresponding word in the original sentence. 
  For example, with appropriate comparators, it will return "cats are lovely" given "cads are lovley"
  '''
  ...
  '''
  Implement IdenticalWordComparator class (a subclass of WordComparator) that matches only the words that 
  are to-the-letter identical. E.g., it should match "cat" and "cat" but should not match "cat" and "cab".
  '''
class IdenticalWordComparator(WordComparator):
  ...
  '''
  Implement OneSymbolDiffComparator class (a subclass of WordComparator) that matches only the words 
  that are different in exactly one letter. E.g., it should match "cat" and "cab" but should not match 
  any of "cat" and "cate", "cat" and "but", or "cat" and "cat".
  '''
class OneSymbolDiffComparator(WordComparator):
  ...
  def match(self, word1, word2):
    ...
    if len(word1) == len(word2):
      for iter in range(len(word1)):
        record.append(True) if word1[iter] == word2(iter) else record.append(False)
  ...
  '''      
  Implement SwapTwoSymbolsComparator class (a subclass of WordComparator) that matches only 
  the words where two consecutive letters are swapped. 
  For example, it should match "cheese" and "cheees", or "cheese and "hceese", 
  but it should not match "cheese" and "echese", or "cheese" and "cheese" 
  (if the word has a double letter, such as "ee" in "cheese", ignore it so that the comparator would not match the word itself).
  '''
class SwapTwoSymbolsComparator(WordComparator):
  def __init__(self):
    pass
  def match(self, word1, word2): #word1 user input whether from sentence or a single word and word2 will be list of known words
    #word1 will be the messed up word
    prev_chr = ""
    iter1 = 0
    iter2 = 2
    if len(word1) == len(word2):
...
x = Spellchecker()
x6 = x.add_word(Noun("cat"))
x1 = x.add_word(UncountableNoun("moose"))
x2 = x.add_word(IrregularNoun("mouse", "mice"))
x3 = x.add_word(Verb("move"))
x4 = x.add_word(IrregularVerb("eat", "ate", "eaten"))
x11 = x.add_word(ToBe())

#x10 = x.delete_word("cat", Noun)
#print(x10)
print(x11)

x12 = x.is_correct_sentence("cats are eating mice")
print(x12)
...
class MatchAllComparator(WordComparator):
    def match(self, word1, word2):
        return True

try:
    sc = Spellchecker()
    comp = [MatchAllComparator()]
    sc.add_word(Noun("band"))
    sc.add_word(Noun("bank"))
    print(sc.autocorrect_word("band", comp) == "band")
    print(sc.autocorrect_word("bank", comp) == "band")
    print(sc.autocorrect_word("sdfkjh", comp) == "band")
...
try:
    ...
    sc.add_word(Noun("band"))
    sc.add_word(Noun("bank"))
    print(sc.autocorrect_sentence("", comp) == "")
    print(sc.autocorrect_sentence("they go to university", comp) == "band band band band")
except Exception as ex:
    print(ex)